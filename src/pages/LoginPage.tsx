...
const DEFAULT_SESSION_DURATION_DAYS = 90; // 3 months default
...
export const LoginPage: React.FC = () => {
  ...
  const handleGoogleLogin = async () => {
    ...
    setErrorMsg(null);
    try {
      await loginWithGoogle(DEFAULT_SESSION_DURATION_DAYS);
      notifications.show({
        title: 'Authentication Successful',
        message: 'Signed in with Google. Session active for 3 months (90 days) by default.',
        color: 'teal',
      });
    } catch (err: unknown) {
        ...
        <Stack gap="lg">
          ...
          <Paper p="xs" radius="md" className={classes.footerPanel}>
            <Group justify="space-around" wrap="nowrap">
              <Tooltip label="Default 3-month session (configurable in Profile & Settings)" withArrow>
                <Group gap={6}>
